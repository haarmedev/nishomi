...
export class ProductdetailsComponent implements OnInit {
  ...
  buyProduct() {
    ...
    if(this.fullLength.value>0 && this.sleeveLength.value>0&&this.bust.value>0&&this.hip.value>0){
      formData.append('Size', 'Size: FullLength: '+this.fullLength.value+' SleeveLength: '+this.sleeveLength.value+' Bust: '+this.bust.value+' Hip: '+this.hip.value);
    }
    else{
    ...
    this.httpClient.post(CUSTOMERREQUEST, formData).subscribe(
      (response: any) => {
        ...
        if (response.data) {
          ...
          Swal.fire('Thank you '+this.profileForm.value.name+' for your interest on '+this.product.name+'. We will contact you soon!!!')
